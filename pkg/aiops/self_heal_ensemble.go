...
// findMatchingPolicy returns the healing policy registered for a decision key.
func (sh *SelfHealEngine) findMatchingPolicy(decision string) *HealingPolicy {
	if p, ok := sh.policies[decision]; ok {
		return &p
	}
	return nil
}

// captureMetricsSnapshot flattens a metrics snapshot into a labelled map
// for inclusion in the decision audit trail.
func captureMetricsSnapshot(snapshot MetricsSnapshot) map[string]float64 {
	return map[string]float64{
		"cpu_util":      snapshot.CPUUtilization,
		"memory_usage":  snapshot.MemoryUsage,
		"disk_io_read":  snapshot.DiskIORead,
		"disk_io_write": snapshot.DiskIOWrite,
		"network_in":    snapshot.NetworkIn,
		"network_out":   snapshot.NetworkOut,
		"error_rate":    snapshot.ErrorRate,
		"latency_p99":   snapshot.LatencyP99,
	}
}
...
func (sh *SelfHealEngine) executeActions(ctx context.Context, snapshot MetricsSnapshot, anomalyScore, confidence float64, decision string) []Action {
	...
	for _, action := range policy.Actions {
		...
		// Log decision
		sh.logDecision(snapshot, action, anomalyScore, confidence, result.Result)
	}
