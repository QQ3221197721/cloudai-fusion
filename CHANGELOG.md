...
### Added — AISecOps Deep Wells (16-Well Fabric)
- `ai/anomaly/mahalanobis.py`: a real **multivariate anomaly detector** (Mahalanobis distance with mean + Ledoit-Wolf-style shrinkage covariance and a chi-square threshold; numpy/scipy only) that catches JOINT anomalies per-metric Z-scores miss; `ai/anomaly/benchmark.py` gives a reproducible labeled benchmark (precision/recall/F1/ROC-AUC) asserted in CI (`tests/test_mahalanobis.py`: AUC 1.00, F1 0.99 on the synthetic set), turning detection quality into a measured fact
- `pkg/hunt/ueba.go`: L2 **UEBA** behavioral hunting — a real, dependency-free statistical engine (Welford online mean/variance + Z-score numeric deviation, per-entity categorical rarity/first-seen), the core method behind Splunk UBA / Exabeam / Elastic ML; `Engine.AnalyzeBehavior` maps anomalies to ATT&CK (T1048/T1078/T1059/T1571), signs a `hunt.behavior` receipt, and escalates on the fabric; `POST /api/v1/hunt/behavior` (train + observe)
- `pkg/intel/stix.go`: a real **STIX 2.1** parser (the OASIS threat-intel standard exported by MISP/OTX/most TIPs) — indicator patterns (ipv4/ipv6-addr, domain-name, url, email-addr, `file:hashes.*`, `=`/`IN`) → IOCs (severity from `x_severity`/`confidence`), `vulnerability` → CVEs, `attack-pattern` → MITRE techniques; ingest via `stix.json` in a feed dir (pull) or `POST /api/v1/intel/stix` (push, `Hub.ImportSTIXBundle`)
- `pkg/detect`: a real, dependency-light **Sigma-compatible detection engine** (the industry detection-rule standard) — parses Sigma YAML (logsource, search identifiers, field modifiers `contains/startswith/endswith/re/cidr/all`, value lists, and the `and/or/not` + `N|all|1 of [them|prefix*]` condition grammar), evaluates structured log events, ships an embedded community-style rule set, and loads the full upstream SigmaHQ corpus at deploy via `LoadSigmaDir`; `POST /api/v1/soc/detect` runs it, storing MITRE-mapped findings that escalate on the fabric
- `pkg/intel` (L1), `pkg/hunt` (L2), `pkg/soc` (L3-L8): a verifiable security-operations overlay — deterministic detectors consuming L1 IOC/CVE intelligence, producing MITRE ATT&CK-mapped findings, escalating to an L8 SOAR orchestrator; every analysis/response is a signed L13 evidence receipt
- `pkg/eventbus/deepwell.go`: EventBus v2 **16-well fabric** — directed connectivity matrix + hop-bounded `WellRouter`, wired in `cmd/apiserver`; an **L8 auto-consumer** turns any L3-L7 detection into an evidence-signed SOAR response automatically (idempotent per finding), closing the detection→response loop
- `pkg/wellreadiness`: per-well honesty instrument (wired / real-backend / fabric-connected / evidence-backed / maturity 0-5) with self-consistency laws; `wellreadiness.Enforce()` fails a production boot on any overclaim; `GET /api/v1/wells` publishes the honest snapshot
- Real backends behind the well interfaces: **L1** ClickHouse HTTP store (`CLOUDAI_CLICKHOUSE_ENDPOINT`), **L3** `/proc` EDR collector on Linux (`CLOUDAI_EDR_REAL_COLLECTOR`), **L8** response actuator — gateway IP-ACL block + active NetworkPolicy — armed by operators via `CLOUDAI_GATEWAY_ENABLE_IP_ACL`
- `GET·POST /api/v1/soc/*` (findings, playbooks, mitigations, analyze/*, collect/endpoint, findings/:id/respond), `POST /api/v1/hunt`, `POST /api/v1/intel/sync`, `GET /api/v1/wells`
- `cmd/cafctl moat-demo` + CI `verifiable-moat` job: runs a REAL red-team engagement, then INDEPENDENTLY re-verifies its exported signed chain OFFLINE against a pinned key and asserts a tampered chain is rejected — L13 verifiability is now a per-commit CI-verified fact
- `docs/aisecops-subsystem-spec.md`: full 16-well design, API surface, and verification
